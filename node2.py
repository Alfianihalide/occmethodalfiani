import pandas as pd
import math
from preprocessing import kriteria_max,subkriteria_maxs,df3,datakriteria,mgr,max_iterasi
#tabel baru
def coba(a,b,c,d,e):
    kriteria_max=a
    subkriteria_maxs=b
    ...
    datakriteria=d
    mgr=e
    df3=df3.loc[df3[kriteria_max[0]]==subkriteria_maxs[0]]
    ...
    writer = pd.ExcelWriter('tabel database iterasi 1.2.xlsx', engine='xlsxwriter')
    df3.to_excel(writer, sheet_name='data 1', index=False)
    writer.save()
...
for val in range(1,max_iterasi):
    kriteria_max,subkriteria_max,df3,datakriteria,mgr,entropy_max,tabelnodes,tabelgs,tblbaris=coba(kriteria_max,subkriteria_maxs,df3,datakriteria,mgr)
    ...
    if entropy_max==0:
        ...
        tblbaris_dic["iterasi {}".format(val+1)]=tblbaris
...
tblbaris1=tblbaris_dic['iterasi 2']
subk21=tblbaris1.nlargest(len(tblbaris1),'entropy')
writer = pd.ExcelWriter('subk21.xlsx', engine='xlsxwriter')
# tabelnodes.to_excel(writer, sheet_name='data 1', index=False)
subk21.to_excel(writer, sheet_name='data')
writer.save()

tblbaris=tblbaris_dic['iterasi 1']
subk2=tblbaris.nlargest(len(tblbaris),'entropy')
writer = pd.ExcelWriter('subk2.xlsx', engine='xlsxwriter')
# tabelnodes.to_excel(writer, sheet_name='data 1', index=False)
subk2.to_excel(writer, sheet_name='data')
writer.save()